...
- 클라이언트는 서버에게 리소스 요청(`requset`)만을 수행하며, 서버는 HTTP를 통해 요청된 데이터를 전달(`response`)하는 것 말고는 클라이언트을 수정하지 않아야 합니다.
...
- 클라이언트와 서버 사이 승인된 중개자 서버에게 연결할 수 는 `Proxy` 서버를 사용할 수 있습니다.
